...
const Header: React.FC = () => {
    ...
    const arrMenu = ['Home', 'Projects', 'Publications', 'About'];
    ...
    return (
      <header className={`fixed md:sticky top-0 left-0 right-0 bg-white p-4 md:px-8 md:py-3 flex justify-between items-center transition duration-300 ${isNavUp ? "-translate-y-full" : ""}`}>
        <a className="logo flex items-center p-4" href={arrHref[0]}>
          <div className="flex items-center">
            <img src={dirLogo} width="32px" alt="Logo" />
            <div className="text-2xl font-semibold pl-3">Hye Soo Park</div>
          </div>
        </a>
        <nav>
          <div
            className={`menu btn2 md:hidden ${isMenuOpen ? 'open' : ''}`}
