    ...
    <div className="prose dark:prose-invert flex flex-wrap gap-4 max-w-4xl divide-y md:divide-y-0 ">
      {allProjects.map((project) => (
        ...
        >
          ...
          ></Image>
          <div
            className="md:absolute relative top-0 w-full h-full 
                        md:opacity-0 hover:opacity-100 transition duration-500 
                        flex flex-col gap-2 md:items-center justify-center md:px-5"
          >
            <div className="absolute top-0 w-full h-full bg-black md:opacity-50 opacity-0">
              {" "}
            </div>
            <div className="flex gap-2 z-20">
              {project.tags &&
                project.tags.map((el, i) => (
                  <kbd key={i} className="bg-main_color text-white px-2">
                    #{el}
                  </kbd>
                ))}
            </div>
            <div className="text-2xl font-bold md:text-white z-20">
              {project.title}
            </div>
            {project.description && (
